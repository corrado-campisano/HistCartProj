...
@Service
public class EventoStoricoValidation {
	
	public FieldErrorResponse validate(EventoStoricoModel model) {
		...
		// TipoEvento
		// --------------------------------------------------------------------
		Long tipoEvento = model.getTipoEvento();
		// obbligatorio, non deve essere null o <=0 
		if (tipoEvento == null || tipoEvento.longValue()<=0) {
			CustomFieldError cfe = new CustomFieldError();
			cfe.setField("tipoEvento");
			cfe.setMessage("Selezionare un TipoEvento");
