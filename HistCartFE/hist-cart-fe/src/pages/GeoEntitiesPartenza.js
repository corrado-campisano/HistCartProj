...
import {vaiAllaHome, vaiAllaMappa, vaiGeoEntityPartenza, vaiLeggendeCorrelate, 
	vaiGeoEntitiesArrivo, vaiEventiStorici} from "../utils/NavigationFunctions.js";
...
export default function GeoEntitiesPartenza() {
			...
			renderHeader: () => (
				<Box display="flex" justifyContent="space-between">
					...
					<Button variant="contained" size="small" startIcon={<AddIcon />} 
						onClick={() => vaiGeoEntityPartenza(navigate, token, 0)}>
	...
	const modifica = async (id) => {
		console.log("Modifica GeoEntityPartenza di ID: " + id);
		vaiGeoEntityPartenza(navigate, token, id);
	};
	
	const elimina = async (id) => {
		console.log("Eliminazione GeoEntityPartenza di ID: " + id);
		...
		if (!label) {
			setAlertMessage("Impossibile recuperare la GeoEntityPartenza di ID" + id);
			return;			
		}		
	...
	const caricaGeoEntitiesPartenza = async () => {
		console.log("Caricamento GeoEntityPartenza");
				
		trackPromise(
			axios.get(Constants.REACT_APP_BACK_END_URL + '/api/geoEntities/list/true', 
				{ headers: { "Authorization": `Bearer ${token}` } })
	...
	return (
		<>
			<Container maxWidth="xl">
				...
				<hr />
				<Typography variant='h5'>{Messages.LBL_LISTA_GEOENTITIES_PARTENZA}</Typography>
				<hr /><br />
