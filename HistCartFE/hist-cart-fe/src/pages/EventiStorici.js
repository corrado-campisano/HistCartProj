...
import {vaiAllaHome, vaiEventoStorico, vaiAllaMappa, vaiLeggendeCorrelate,  
...
export default function EventiStorici() {
			...
			renderHeader: () => (
				<Box display="flex" justifyContent="space-between">
					...
					<Button variant="contained" size="small" startIcon={<AddIcon />} 
						onClick={() => vaiEventoStorico(navigate, token, 0)}>
	...
	const modifica = async (id) => {
		console.log("Modifica EventoStorico di ID: " + id);
		vaiEventoStorico(navigate, token, id);
	};
	
	const elimina = async (id) => {
		console.log("Eliminazione EventoStorico di ID: " + id);
		...
		if (!label) {
			setAlertMessage("Impossibile recuperare l'EventoStorico di ID" + id);
			return;			
		}		
		if (!window.confirm(Messages.MSG_DEL_CONF_EVENTO_STORICO + " '" + label + "' ?")) {
			return;
		}
		
		trackPromise(
			axios.delete(Constants.REACT_APP_BACK_END_URL + '/api/eventiStorici/' + id, 
	...
	return (
		<>
			<Container maxWidth="xl">
				...
				<hr />
				<Typography variant='h5'>{Messages.LBL_LISTA_EVENTI_STORICI}</Typography>
				<hr /><br />
