...
import { vaiGeoEntityPartenza } from "../utils/NavigationFunctions.js";
...
export default function GeoEntityPartenza() {
	...
	const [eventoStoricoFuturoFk, setEventoStoricoFuturoFk] = useState('');
	const handleChangeEventoStoricoFuturoFk = (event) => {
		setEventoStoricoFuturoFk(event.target.value);
	};
	const [eventoStoricoFuturoFkError, setEventoStoricoFuturoFkError] = useState('');
	...
	const caricaGeoEntityPartenza = async () => {
		console.log("Caricamento GeoEntityPartenza");
		...
		trackPromise(
			...
			.then(response => {
				...
				if (response.data) {
					...
					closePolygon();
					setEventoStoricoFuturoFk(response.data.eventoStoricoFuturoFk);
	...
	const creaNuova = async () => {
		console.log("Creazione nuova GeoEntityPartenza");
		...
		const nuovaGeoEntityPartenza = {
			id: id,
			partenzaAutArrivo: true, 
			descrizione: descrizione,
			wikiLink: wikiLink,
			polygon: polygon,
			eventoStoricoFuturoFk: eventoStoricoFuturoFk,
		};
		//console.log("nuovaGeoEntityPartenza: " + JSON.stringify(nuovaGeoEntityPartenza));
		...
		functionArray["eventoStoricoFuturoFk"] = setEventoStoricoFuturoFkError;
		
		trackPromise(
			axios.post(Constants.REACT_APP_BACK_END_URL + '/api/geoEntities/', nuovaGeoEntityPartenza, 
				{ headers: { "Authorization": `Bearer ${token}` } })
			...
			.then(response => {
				...
				if (response.data) {
					...
					if (id==="0" && response.data.entityId) {
						vaiGeoEntityPartenza(navigate, token, response.data.entityId);
					}
	...
	useEffect(() => {
		...
		if (id!=="0") {
			setPageTitle(Messages.MSG_MOD_GEOENTITY_PARTENZA);
			setButtonTitle(Messages.BTN_SALVA);
			
			caricaGeoEntityPartenza();
			
		} else {
			setPageTitle(Messages.MSG_NEW_GEOENTITY_PARTENZA);
				...
				<Grid container spacing={2} alignItems="flex-end" >
					...
					<Grid item xs={6}>
						<Grid container spacing={2} alignItems="flex-end" >
							...
							<Grid item xs={12}>
								<InputLabel required id="eventoStoricoFuturoFk-label">Evento Futuro:</InputLabel>
								<Select
									labelId="eventoStoricoFuturoFk-label"
									id="eventoStoricoFuturoFk" name="eventoStoricoFuturoFk" 
									value={eventoStoricoFuturoFk || []} fullWidth
									label="Evento Storico Futuro" size="small" 
									onChange={handleChangeEventoStoricoFuturoFk}
								>
									{listEventiStorici}
								</Select>
								{eventoStoricoFuturoFkError !== "" && <FormHelperText style={{ color: COLORS.red_error }}>{eventoStoricoFuturoFkError}</FormHelperText>}
							</Grid>
