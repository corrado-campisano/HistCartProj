...
import {vaiAllaHome, vaiAllaMappa, vaiLeggendaCorrelata, vaiGeoEntitiesPartenza, 
...
export default function LeggendeCorrelate() {
			...
			renderHeader: () => (
				<Box display="flex" justifyContent="space-between">
					...
					<Button variant="contained" size="small" startIcon={<AddIcon />} 
						onClick={() => vaiLeggendaCorrelata(navigate, token, 0)}>
	...
	const modifica = async (id) => {
		console.log("Modifica LeggendaCorrelata di ID: " + id);
		vaiLeggendaCorrelata(navigate, token, id);
	};
	
	const elimina = async (id) => {
		console.log("Eliminazione LeggendaCorrelata di ID: " + id);
		...
		if (!label) {
			setAlertMessage("Impossibile recuperare la LeggendaCorrelata di ID" + id);
			return;			
		}		
		...
		trackPromise(
			axios.delete(Constants.REACT_APP_BACK_END_URL + '/api/leggendeCorrelate/' + id, 
	...
	return (
		<>
			<Container maxWidth="xl">
				...
				<hr />
				<Typography variant='h5'>{Messages.LBL_LISTA_LEGGENDE_CORRELATE}</Typography>
				<hr /><br />
